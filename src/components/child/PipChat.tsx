...
export function PipChat({ taskId }: { taskId: string }) {
  ...
  async function send(text?: string) {
    ...
    // Also guards a second chip tap while the first reply is in flight.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "linear-gradient(180deg,#F6F2FF,#EEF3FE 40%)" }}>
      ...
      {/* The quick replies used to live at the bottom of the scrolling message
          list, below the element we auto-scroll to — so once a conversation
          started they sat off-screen and looked broken. They belong with the
          composer, pinned above it and always reachable. */}
      <div style={{ display: "flex", gap: 8, padding: "0 14px 2px", overflowX: "auto", flex: "none" }}>
